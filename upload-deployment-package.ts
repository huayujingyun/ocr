...
async function uploadDeploymentPackage() {
  console.log("========================================");
  console.log("开始上传部署包到对象存储");
  console.log("========================================\n");

  try {
    // 文件路径
    const tarFilePath = "/workspace/projects/card-ocr-deployment-v1.0.0.tar.gz";
    ...
    const fileKey = await storage.uploadFile({
      ...
      fileName: "card-ocr-deployment-v1.0.0.tar.gz",
    ...
    console.log("📋 文件信息：");
    console.log(`  文件名: card-ocr-deployment-v1.0.0.tar.gz`);
    ...
    console.log("4. 运行install.bat（管理员）");
    ...
    // 保存下载链接到文件
    const linkFile = "/workspace/projects/download-link.txt";
    const linkContent = `购物卡OCR识别系统 - Windows一键部署包
...
文件信息：
- 文件名：card-ocr-deployment-v1.0.0.tar.gz
...
`;
    ...
    console.log("✓ 下载链接已保存到: download-link.txt");
